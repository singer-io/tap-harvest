...
#         LOGGER.warn("A user with the email {} already exists. Removing user...")
...
#         response = requests.post(url="https://api.harvestapp.com/v2/users", headers=HEADERS, json=data)
#         if response.status_code >= 400:
#             LOGGER.warn("Unable to delete this user")
#             LOGGER.warn("create_user: {} {}".format(response.status_code, response.text))
#             assert None
...
def create_client():
    ...
    if response.status_code >= 400:
        if data['name'] in [client['name'] for client in  get_all('clients')]:
            ...
            response = requests.post(url="https://api.harvestapp.com/v2/clients", headers=HEADERS, json=data)
            if response.status_code >= 400:
                LOGGER.warn("create_client: {} {}".format(response.status_code, response.text))
                assert None
            return response.json()
        LOGGER.warn("create_client: {} {}".format(response.status_code, response.text))
        assert None
...
def create_contact(client_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_contact: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_estimate(client_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_estimate: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_estimate_item_category():
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_estimate_item_category: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_estimate_message(estimate_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_estimate_message: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_expense(project_id):
    ...
    #     if response.status_code >= 400:
    #         LOGGER.warn("create_expense: {} {}".format(response.status_code, response.text))
    #         assert None
...
def create_expense_category():
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_expense_category: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_invoice(client_id, estimate_id: str = "", project_id: str = ""):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_invoice: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_invoice_message(invoice_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_invoice_message: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_invoice_payment(invoice_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_invoice_payment: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_invoice_item_category():
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_invoice_item_category: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_project(client_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_project: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_project_task(project_id, task_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_task_assignment: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_project_user(project_id, user_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('create_project_user: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_role():
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_roles: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_task():
    ...
    if response.status_code >= 400:
        LOGGER.warn("create_task: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()
...
def create_time_entry(project_id, task_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('create_time_entry: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_client(client_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_client: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_contact(contact_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_contact: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_estimate(estimate_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_estimate: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
#     response = requests.patch(url="https://api.harvestapp.com/v2/estimates/{}".format(estimate_id), headers=HEADERS, json=data)
#     if response.status_code >= 400:
#         LOGGER.warn('update_estimate_line_items: {} {}'.format(response.status_code, response.text))
#         assert None
#     return response.json()
...
def update_estimate_message(estimate_id):
    ...
    response = requests.post(url="https://api.harvestapp.com/v2/estimates/{}/messages".format(estimate_id), headers=HEADERS, json=data)
    if response.status_code >= 400:
        LOGGER.warn('update_estimate_message: {} {}'.format(response.status_code, response.text))
...
def update_estimate_item_category(category_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_estimate_item_category: {} {}'.format(response.status_code, response.text))
...
def update_expense(expense_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_expense: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_expense_category(category_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_expense_category: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_invoice(invoice_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_invoice: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_invoice_message(invoice_id):
    ...
    response = requests.post(url="https://api.harvestapp.com/v2/invoices/{}/messages".format(invoice_id), headers=HEADERS, json=data)
    if response.status_code >= 400:
        LOGGER.warn('update_invoice_message: {} {}'.format(response.status_code, response.text))
...
def update_invoice_item_category(category_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_invoice_item_category: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_project_task(project_id, task_assignment_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('ERROR in Updating task {}'.format(task_id))
        assert None
    return response.json()
...
def update_project_user(project_id, project_user_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_project_user: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_role(role_id, user_ids = None):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_role: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def update_task(task_id):
    ...
    if response.status_code >= 400:
        ...
        response = requests.patch(url="https://api.harvestapp.com/v2/tasks/{}".format(task_id), headers=HEADERS, json=data)
        if response.status_code >= 400:
            LOGGER.warn('update_task: {} {}'.format(response.status_code, response.text))
            assert None
    return response.json()
...
def update_time_entry(time_entry_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_time_entry: {} {}'.format(response.status_code, response.text))
        data = {"notes":"Updated Time Entry Note {}".format(random.randint(0,1000000)),
                "external_reference":{"id":random.randint(5, 1000000),"group_id":random.randint(100,500),
                                      "permalink":"https://help.getharvest.com/api-v2/timesheets-api/timesheets/time-entries/"}}
        response = requests.patch(url="https://api.harvestapp.com/v2/time_entries/{}".format(time_entry_id), headers=HEADERS, json=data)
...
def update_user(user_id, role_names = None):
    ...
    if response.status_code >= 400:
        LOGGER.warn('update_user: {} {}'.format(response.status_code, response.text))
        assert None
    return response.json()
...
def get_all(stream):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_{}: {} {}".format(stream, response.status_code, response.text))
        assert None
...
def get_user_projects(user_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_project_users: {} {}".format(stream, response.status_code, response.text))
        assert None
...
def get_all_estimate_messages(estimate_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_estimate_messages: {} {}".format(response.status_code, response.text))
        assert None
    return response.json()[stream]
...
def get_all_invoice_messages(invoice_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_invoice_messages: {} {}".format(response.status_code, response.text))
        assert None
...
def get_all_invoice_payments(invoice_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_invoice_payments: {} {}".format(response.status_code, response.text))
        assert None
...
def get_all_project_tasks(project_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_all_project_tasks: {} {}".format(response.status_code, response.text))
        assert None
...
def get_stream_counts(stream, parameters = [("updated_since", UPDATED_SINCE)]):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_{}_count: {} {}".format(stream, response.status_code, response.text))
        assert None
...
def get_random(stream):
    ...
    response = requests.get(url="https://api.harvestapp.com/v2/{}".format(stream), headers=HEADERS)
    if response.status_code >= 400:
        LOGGER.warn("get_random_{}: {} {}".format(stream, response.status_code, response.text))
        assert None
...
def get_random_project_user(project_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("get_random_project_user: {} {}".format(response.status_code, response.text))
        assert None
...
def get_random_project_task(project_id):
    response = requests.get(url="https://api.harvestapp.com/v2/projects/{}/task_assignments".format(project_id), headers=HEADERS)
    if response.status_code >= 400:
        LOGGER.warn("get_random_project_taks: {} {}".format(response.status_code, response.text))
        assert None
    num_project_tasks = len(response.json()['task_assignments']) -1
    return response.json()['task_assignments'][random.randint(0, num_project_tasks)]['id']
...
def get_random_task(project_id: str = None):
    if project_id:
        response = requests.get(url="https://api.harvestapp.com/v2/projects/{}/task_assignments".format(project_id), headers=HEADERS)
        if response.status_code >= 400:
            LOGGER.warn("get_random_task: {} {}".format(response.status_code, response.text))
            assert None
        num_project_tasks = len(response.json()['task_assignments']) -1
...
def delete_stream(stream, stream_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("delete_{}: {} {}".format(stream, response.status_code, response.text))
    return response.json()
...
def delete_project_user(project_id, project_user_id):
    ...
    if response.status_code >= 400:
        LOGGER.warn("delete_project_user: {} {}".format(response.status_code, response.text))
    return response.json()
