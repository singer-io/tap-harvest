...
class TestStartDateHonoring(BaseTapTest):
    ...
    def setUpClass(cls):
        ...
        # Create dummy data in the specific streams
        for itter in range(3):
            ...
            cls._teardown_delete["estimate_messages"].append(
                {"id": estimate_message_1["id"]}
            )

            invoice_1 = create_invoice(
                client_id=client_1["id"], project_id=project_1["id"]
            )
            ...
            cls._teardown_delete["invoice_messages"].append(
                {"id": invoice_message_1["id"]}
            )
            cls._teardown_delete["invoice_payments"].append(
                {"id": invoice_payment_1["id"]}
            )
    ...
    def get_second_updated_parent_id(self, records, stream):
        ...
        highest_updated_at = (
            records.get(stream).get("messages")[0].get("data").get("updated_at")
        )
