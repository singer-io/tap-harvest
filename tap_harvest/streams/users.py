...
class Users(ParentBaseStream):
    ...
    def get_bookmark(self, state: Dict, stream: str, key: Any = None) -> int:
        ...
        min_parent_bookmark = (
            super().get_bookmark(state, stream)
            if self.is_selected()
            else self.client.config["start_date"]
        )
        ...
        user_projects_bookmark = (
            super().get_bookmark(
                state, user_project_obj.tap_stream_id, key=parent_bookmark_key
            )
            if user_project_obj.is_selected()
            else self.client.config["start_date"]
        )
