...
def sync(client: Client, config: Dict, catalog: singer.Catalog, state) -> None:
    ...
    with singer.Transformer() as transformer:
        for stream_name in selected_streams:
            ...
            stream.write_schema(stream_schema)
            for child in stream.children:
                if child in selected_streams:
                    child_obj = STREAMS[child](client)
                    child_schema = catalog.get_stream(child).schema.to_dict()
                    child_obj.write_schema(child_schema)
